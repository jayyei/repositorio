...
CREATE TABLE `Usuarios`(
    ...
    PRIMARY KEY (`id`),
	UNIQUE KEY `username_UNIQUE` (`username`), -- la clausula unique key nos asegura que todos los valores en una columna son diferentes
    UNIQUE KEY `email_UNIQUE` (`password`) -- Tanto unique como primary key nos aseguran que estos campos tendran valores unicos, sin embargo tu podras tener varios campos unicos por tabla pero solo una primary key en la misma
)ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `Vacantes`(
    ...
    PRIMARY KEY(`id`),
    KEY `fk_vacantes_categorias_idx` (`idCategoria`), -- key es un sinonimo de index solo para compatibilidad con otras bd
